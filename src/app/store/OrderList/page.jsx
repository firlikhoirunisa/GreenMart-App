...
const GreenMartOrderManagement = () => {
  ...
  const [orders, setOrders] = useState([
    ...
    },
    {
      ...
      items: [{ name: "Pakcoy", qty: 4, price: 5000 }],
      total: 20000,
      status: "shipped",
      paymentMethod: "COD",
    ...
    },
    {
      ...
      items: [{ name: "Cabai Rawit", qty: 5, price: 500 }],
      total: 2500,
      status: "cancelled",
      paymentMethod: "COD",
  ...
  const handleSearch = (e) => {
    if (e.key === "Enter") {
      ...
      router.push(`/orders/search?q=${encodeURIComponent(searchValue)}`);
    }
  };
  ...
  const handleStatusUpdate = (orderId, newStatus) => {
    ...
    // You could also navigate to a confirmation page or show success message
    // router.push(`/orders/${orderId}/status-updated`);
  };

  const handleViewDetail = (orderId) => {
    // Navigate to order detail page
    router.push(`/orders/detail/${orderId.replace("#", "")}`);
  };

  const handlePrintLabel = (orderId) => {
    // Navigate to print label page
    router.push(`/orders/print-label/${orderId.replace("#", "")}`);
  };
  ...
  const handleBulkAction = (action) => {
    ...
    // Navigate to bulk action page with selected orders
    const orderIds = selectedOrders.join(",");
    if (action === "Update Status") {
      router.push(`/orders/bulk-update?orders=${orderIds}`);
    } else if (action === "Print Labels") {
      router.push(`/orders/bulk-print?orders=${orderIds}`);
    }
  };
